          ...
          {(date) => (
            <CalendarCell
              ...
              className={composeRenderProps(className, (className, { isSelected, isDisabled }) =>
                twMerge(
                  "relative flex size-10 cursor-default items-center justify-center rounded-lg text-neutral-content tabular-nums outline-hidden hover:bg-secondary-fg/15 sm:size-9 sm:text-sm/6 forced-colors:text-[ButtonText] forced-colors:outline-0",
                  isSelected &&
                    "bg-primary pressed:bg-primary text-primary-content hover:bg-primary/90 data-invalid:bg-danger data-invalid:text-danger-fg forced-colors:bg-[Highlight] forced-colors:text-[Highlight] forced-colors:data-invalid:bg-[Mark]",
                  isDisabled && "text-neutral-content forced-colors:text-[GrayText]",
                  date.compare(now) === 0 &&
...
const CalendarHeader = ({
  ...
  return (
    <header
      ...
      className={twMerge(
        "flex w-full justify-center gap-1.5 pt-1 pe-1 pb-5 ps-1.5 sm:pb-4",
        className,
      )}
      {...props}
    >
      ...
      <Heading
        className={twMerge(
          "me-2 flex-1 text-start font-medium text-neutral-content sm:text-sm",
          !isRange && "sr-only",
          className,
        )}
      />
      <div className="flex items-center gap-1">
        <Button
          size="square-petite"
          className="size-8 **:data-[slot=icon]:text-neutral-content sm:size-7"
          shape="circle"
          intent="plain"
          slot="previous"
        >
...
const CalendarGridHeader = () => {
  return (
    <CalendarGridHeaderPrimitive>
      {(day) => (
        <CalendarHeaderCell className="pb-2 font-semibold text-neutral-content text-sm sm:px-0 sm:py-0.5 lg:text-xs">
          {day}
        </CalendarHeaderCell>
      )}
    </CalendarGridHeaderPrimitive>
