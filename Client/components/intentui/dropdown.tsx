...
const dropdownItemStyles = tv({
  base: [
    ...
    "group relative cursor-default select-none rounded-[calc(var(--radius-lg)-1px)] px-[calc(var(--spacing)*2.3)] py-[calc(var(--spacing)*1.3)] forced-color:text-[Highlight] text-base text-neutral-content outline-0 forced-color-adjust-none sm:text-sm/6 forced-colors:text-[LinkText]",
    "**:data-[slot=avatar]:*:me-2 **:data-[slot=avatar]:*:size-6 **:data-[slot=avatar]:me-2 **:data-[slot=avatar]:size-6 sm:**:data-[slot=avatar]:*:size-5 sm:**:data-[slot=avatar]:size-5",
    "data-danger:**:data-[slot=icon]:text-danger/60 **:data-[slot=icon]:size-4 **:data-[slot=icon]:shrink-0 **:data-[slot=icon]:text-neutral-content focus:data-danger:**:data-[slot=icon]:text-danger",
    "data-[slot=menu-radio]:*:data-[slot=icon]:size-3 *:data-[slot=icon]:me-2",
    "forced-colors:**:data-[slot=icon]:text-[CanvasText] forced-colors:group-focus:**:data-[slot=icon]:text-[Canvas] ",
    "[&>[slot=label]+[data-slot=icon]]:absolute [&>[slot=label]+[data-slot=icon]]:end-0",
  ],
  variants: {
    isDisabled: {
      true: "text-neutral-content forced-colors:text-[GrayText]",
    },
    isSelected: {
    ...
    isFocused: {
      ...
      true: [
        "**:data-[slot=icon]:text-primary-content **:[kbd]:text-primary-content",
        ...
        "data-[slot=description]:text-primary-content data-[slot=label]:text-primary-content [&_.text-neutral-content]:text-primary-content/80",
      ],
    },
  },
})
...
const dropdownSectionStyles = tv({
  slots: {
    ...
    header: "col-span-full px-2.5 py-1 font-medium text-neutral-content text-sm sm:text-xs",
  },
})
    ...
    >
      ...
      {description && (
        <Text
          ...
          className={twMerge("text-neutral-content text-xs", classNames?.description)}
          {...restProps}
        >
          {description}
        </Text>
...
const DropdownKeyboard = ({ className, ...props }: React.ComponentProps<typeof Keyboard>) => {
  return (
    <Keyboard
      classNames={{
        base: twMerge(
          "absolute end-2 group-hover:text-primary-fg group-focus:text-primary-fg ps-2",
          className,
        ),
      }}
      {...props}
