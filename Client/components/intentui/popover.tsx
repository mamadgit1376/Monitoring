...
const content = tv({
  base: [
    "peer/popover-content !bg-gray-800 max-w-xs rounded-xl border bg-overlay bg-clip-padding text-overlay-fg shadow-xs transition-transform [scrollbar-width:thin] sm:max-w-3xl sm:text-sm dark:backdrop-saturate-200 forced-colors:bg-[Canvas] [&::-webkit-scrollbar]:size-0.5",
  ],
  variants: {
    isPicker: {
    ...
    isEntering: {
      true: [
        ...
        "data-[placement=left]:slide-in-from-end-1 data-[placement=right]:slide-in-from-start-1 data-[placement=top]:slide-in-from-bottom-1 data-[placement=bottom]:slide-in-from-top-1",
      ],
    },
    isExiting: {
      true: [
        "fade-out animate-out duration-100 ease-in",
        "data-[placement=left]:slide-out-to-end-1 data-[placement=right]:slide-out-to-start-1 data-[placement=top]:slide-out-to-bottom-1 data-[placement=bottom]:slide-out-to-top-1",
      ],
    },
  },
})
...
const PopoverContent = ({
  ...
  return isMobile && respectScreen ? (
    <ModalOverlay
      className="fixed top-0 start-0 isolate z-50 h-(--visual-viewport-height) w-full bg-overlay/10 [--visual-viewport-vertical-padding:16px]"
      {...props}
      isDismissable
    >
      <Modal
