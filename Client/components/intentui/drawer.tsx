      ...
      {(props?.isOpen || state?.isOpen) && (
        ...
        >
          {({ state }) => (
            <Modal
              className={twJoin(
                "fixed max-h-full touch-none overflow-hidden bg-bg align-middle text-neutral-content shadow-sm will-change-transform",
                side === "top" &&
                  `${
                    isFloating
                ...
                side === "right" &&
                  `w-full max-w-xs overflow-y-auto **:[[slot=header]]:text-start ${
                    isFloating
...
const DrawerDescription = ({ className, ...props }: TextProps) => (
  <Text slot="description" className={twMerge("text-neutral-content text-sm", className)} {...props} />
)
